...
async function qpay(method, url, body) {
  ...
  try {
    ...
    if (err.response?.status === 401) { _token = null; _tokenExpiresAt = 0; }
    throw err;
  }
}
...
async function markOrderPaid(orderId, amountMnt, qpayPaymentId) {
  return shopifyApi('POST', `/orders/${orderId}/transactions.json`, {
    transaction: { kind: 'capture', status: 'success', amount: String(amountMnt), currency: 'MNT', gateway: 'QPay', authorization: qpayPaymentId },
  });
}
...
app.get('/auth/shopify/install', (req, res) => {
  ...
  const url = `https://${SHOPIFY_DOMAIN}/admin/oauth/authorize?client_id=${SHOPIFY_API_KEY}&scope=read_orders,write_orders&redirect_uri=${encodeURIComponent(redirectUri)}&state=${state}&grant_options[]=`;
  res.redirect(url);
});
...
app.get('/auth/shopify/callback', async (req, res) => {
  try {
    ...
    if (!SHOPIFY_API_KEY || !SHOPIFY_API_SECRET) return res.status(500).send('SHOPIFY_API_KEY/SECRET тохируулаагүй');
    ...
    res.send(`<!doctype html><body style="font-family:system-ui;max-width:600px;margin:40px auto;padding:24px"><h2>Shopify Admin API token бэлэн</h2><pre style="background:#f4f4f5;padding:12px;border-radius:8px;word-break:break-all;user-select:all">${token}</pre></body>`);
  } catch (err) {
...
app.get('/api/order/:orderId', async (req, res) => {
  try {
    ...
    const o = data.order;
    res.json({ id: o.id, name: o.name, total_price: o.total_price, currency: o.currency, financial_status: o.financial_status, customer_email: o.email });
  } catch (err) {
...
app.post('/api/invoice/create', async (req, res) => {
  try {
    ...
    if (order.financial_status === 'paid') return res.status(409).json({ error: 'already paid' });
    ...
    const invoice = await qpay('POST', '/v2/invoice', {
      ...
      invoice_description: `${order.name || 'Order'} drift-ub`,
      amount,
      callback_url: callbackUrl,
    });
    invoiceStore.set(invoice.invoice_id, { order_id, amount, created_at: Date.now() });
    res.json({ invoice_id: invoice.invoice_id, qr_text: invoice.qr_text, qr_image: invoice.qr_image, urls: invoice.urls, amount });
  } catch (err) {
...
app.get('/api/invoice/:invoiceId/status', async (req, res) => {
  try {
    ...
    res.json({ paid: !!paidRow, payment_id: paidRow?.payment_id, paid_amount: paidRow?.payment_amount });
  } catch (err) {
...
app.all('/api/qpay/callback', async (req, res) => {
  ...
  try {
    ...
    if (Math.round(paidRow.payment_amount) < info.amount) {
      console.warn(`callback: order ${orderId} дутуу төлөгдсөн`); return;
    }
    await markOrderPaid(orderId, paidRow.payment_amount, paidRow.payment_id);
    console.log(`[ok] order ${orderId} PAID`);
...
app.listen(PORT, () => {
  console.log(`QPay Shopify backend ${PORT}`);
